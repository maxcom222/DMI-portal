            ...
            <table class="table table-bordered table-striped {{ count($users) > 0 ? 'datatable' : '' }} dt-select">
                ...
                @if (count($users) > 0)
                    @foreach ($users as $user)
                        @if(! Gate::allows('users_manage') && ($user->roles()->pluck('name')[0] == 'administrator' || $user->roles()->pluck('name')[0] == 'super admin'))
                            ...
                            <tr data-entry-id="{{ $user->id }}">
                                ...
                                <td>
                                    @foreach ($user->roles()->pluck('name') as $role)
                                        <span class="label label-info label-many">{{ $role=="customer"?"field":$role }}</span>
                                    @endforeach
                                </td>
                                @if(Auth::user()->roles()->pluck('name')[0] == 'administrator'
                                 || Auth::user()->roles()->pluck('name')[0] == 'super admin')
